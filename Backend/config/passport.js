...
passport.deserializeUser(async (id, cb) => {
  // This runs on every request after the initial login.
  // It takes the ID from the cookie and retrieves the full user from the DB.
  try {
    ...
    if (user) {
      // SUCCESS: User found. 
      // result.rows[0] becomes 'req.user' in all your route handlers.
      cb(null, user);
    } else {
      // FAIL: ID in cookie doesn't match any DB record (e.g., account deleted).
      console.log("❌ STEP 2 ERROR: User ID not found in database.");
      cb(null, false); 
    }
  } catch (err) {
    // CRASH: Database is down or query is malformed.
